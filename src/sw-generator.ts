...
const SAFE_IDENTS = new Set(
  (
    ...
    'clearInterval globalThis structuredClone import export default from as'
  ).split(' '),
);
...
function extractParamNames(fnStr: string): Set<string> {
...
export function validateHandler(
  ...
  if (str.includes('[native code]')) {
    throw new Error(
      `phantom-page: route "${pattern}" handler is not serializable (native/bound function)`,
    );
  }
  ...
  const locals = new Set([
    ...[
      ...
      (m) =>
        (m[1] ?? m[2] ?? '').match(/[a-zA-Z_$][a-zA-Z0-9_$]*/g) ?? [],
    ),
  ...
  if (freeVars.length > 0) {
    throw new Error(
      `phantom-page: route "${pattern}" handler references external variables: ${freeVars.join(', ')}. ` +
